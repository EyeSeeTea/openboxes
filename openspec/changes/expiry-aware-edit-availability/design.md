...
| `src/main/groovy/org/pih/warehouse/custom/outboundExpiryRestrictions/support/ExpiryRule.groovy` | Add static helper `sumPickableQuantity(List<AvailableItem>)` that hoists `today` once and reuses the in-class `isExpired` predicate — keeps the upstream `StockMovementService` change to a single method call on a leaf helper, so the upstream file does not depend on the bean-replacement subclass. Custom file from the prior change. | 1 added method (~7 lines). |
