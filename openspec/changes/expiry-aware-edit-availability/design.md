...
|---|---|---|
| `grails-app/services/org/pih/warehouse/inventory/StockMovementService.groovy` | Add `import` for `StockMovementServiceWithExpiryFilter` (custom helper class), one-line `quantityPickable = StockMovementServiceWithExpiryFilter.sumPickableQuantity(availableItems)` alongside the existing `quantityAvailable`/`quantityOnHand` (around line 1050), one entry in the row map literal (around line 1054). | 3 added lines (1 import + 1 expression + 1 map entry). |
...
| `src/main/groovy/org/pih/warehouse/custom/outboundExpiryRestrictions/service/StockMovementServiceWithExpiryFilter.groovy` | Add static helper `sumPickableQuantity(List<AvailableItem>)` that hoists `today` once and reuses `ExpiryRule.isExpired` — keeps the upstream `StockMovementService` change to a single method call. Custom file from the prior change. | 1 added method (~7 lines). |
...
| `grails-app/i18n/custom/outboundExpiryRestrictions-messages.properties` | Add new key `outboundExpiryRestrictions.edit.expiredHint=({0} expired)`. Custom file from the prior change. | 1 added line. |
