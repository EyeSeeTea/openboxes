...
const FIELDS = {
  ...
  availableItems: {
    ...
    getDynamicRowAttr: ({ rowValues }) => {
      const expired = isRowExpired(rowValues);
      let className = '';
      if (!rowValues.quantityAvailable) {
        className = 'text-disabled';
      }
      if (expired) {
        className = `${className} ${expiredRowClassName}`.trim();
      }
      return { className };
    },
    fields: {
...
class ExpiryAwareEditPickModal extends Component {
  constructor(props) {
    super(props);
...
export default connect(mapStateToProps, { showSpinner, hideSpinner })(ExpiryAwareEditPickModal);

ExpiryAwareEditPickModal.propTypes = {
